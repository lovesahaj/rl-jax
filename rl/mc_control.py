...
def train(num_episodes=2000):
    ...
    visited = float(jnp.sum(env.q_table != -1.0))
    total = env.q_table.size
    print(f"Q-table coverage: {visited}/{total} = {100*visited/total:.1f}%")

    return episode_rewards, env


def benchmark(trained_env, train_rewards=None, num_episodes=100, max_steps=500):
    """Evaluate the greedy policy of a trained MC-control agent."""
    ...
    summary = evaluate(
        ...
        policy_factory=lambda env: (lambda e: int(e.choose_action())),
    ...
    save_results(os.path.join(OUTPUT_DIR, "mc_control_metrics.npz"), [summary])
    print(f"MC-control benchmark saved to {OUTPUT_DIR}/mc_control_metrics.npz")
    print(
